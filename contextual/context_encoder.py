...
# x = (ContextEncoder.encode_context_to_vec(
#    {'23': 23, '33:464068': 1, '18': 0, '33:44507': 1, '33:491611': 8, '6': 9, '33:40853': 1, '42': 0, '2:1': 11,
#     '33:2844': 1}))
#
...
class ContextEncoder:
    ...
    def store_prop_value_pair_to_redis(cls, prop, value):
        r = redis.pipeline()
        property_idx = r.incr(cls.CURRENT_IDX_KEY)
        r.set(cls.build_context_encoder_key(prop, value), property_idx)
        r.execute()

    @classmethod
    def encode_context_to_vec(cls, context):
        vec = numpy.zeros(KmeansSettings.KMEANS_DIM)  # [0,0,0] of size max_encoded_idx
        for key, value in context.items():
            ...
            if prop_name in Context.CLUSTERING_PROPERTIES:
                property_idx = int(redis.get(cls.build_context_encoder_key(prop, value)))
                if property_idx:
                    # cache hit
                    pass
                else:
                    # cache miss
                    cls.store_prop_value_pair_to_redis(prop, value)
                vec[property_idx] = 1
        return vec

    @staticmethod
    def build_context_encoder_key(prop, val):
        return 'context_encoder' + ':' + str(prop) + ':' + str(val)
